...
from systeme_etudiant_app.models import Subjects, SessionYearModel, Students, Attendance, AttendanceReport, Staffs, \
    LeaveReportStaff, FeedBackStaffs, CustomUser, Courses, NotificationStaffs, StudentResult



def staff_home(request):
    #for fetch all students Under me
    ...
    #Fetch Attendance Data Approve Subject
    subject_list=[]
    attendance_list=[]
    for subject in subjects:
...
def save_attendance_data(request):
    ...
    # Récupération des objets Model
    subject_model = Subjects.objects.get(id=subject_id)
    session_model = SessionYearModel.objects.get(id=session_year_id)

    # Chargement des IDs étudiants depuis le JSON
    json_student = json.loads(student_ids)
    print(f"Nombre d'étudiants: {len(json_student)}")

    #
    # Création de l'enregistrement d'Assiduité (Attendance) global
    ...
    try:
        ...
        # Itération sur les IDs étudiants et création des rapports d'Assiduité (AttendanceReport)
        for stud in json_student:
            student = Students.objects.get(admin_id=stud['id'])  # 'admin__id' pour lier à l'utilisateur
            attendance_report = AttendanceReport(
                student_id=student,
                attendance_id=attendance,  # L'ID de l'enregistrement créé ci-dessus
                status=stud['status']
            )
...
def staff_feedback_save(request):
    if request.method!= "POST":
        return HttpResponseRedirect(reverse("staff_feedback_leave"))
    else:
...
def profile_staff(request):
    ...
    return render(request,"staff_template/profile_staff_template.html",{"user":user,"staff":staff})
...
def staff_add_result(request):
    subjects=Subjects.objects.filter(staff_id=request.user.id)
    session_years=SessionYearModel.objects.all()
    return render(request,"staff_template/add_staff_result.html",{"subjects":subjects,"session_years":session_years})



def student_save_result(request):
    if request.method!='POST':
        return HttpResponseRedirect('staff_add_result')
    student_admin_id=request.POST.get('student_id')
    assignment_marks = float(request.POST.get('assignment_marks', 0))
    exam_marks = float(request.POST.get('exam_marks', 0))
    subject_id=request.POST.get('subject_id')
...
@csrf_exempt
def student_result_fetch(request):
    subject_id=request.POST.get("subject_id")
